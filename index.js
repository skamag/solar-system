...
// Astronaut
const astronautPivot = new THREE.Group(); // Create a pivot point
const orbitRadius = 2.75; // Adjust orbit radius
const orbitSpeed = 0.001; // Adjust orbit speed

const raycaster = new THREE.Raycaster();
const mouse = new THREE.Vector2();
let astronaut; // Store the astronaut reference

const gltfLoader = new GLTFLoader();
gltfLoader.load("./assets/Astronaut.glb", (gltf) => {
  //   console.log(gltf);
  astronaut = gltf.scene;
  astronaut.scale.set(0.5, 0.5, 0.5); // Adjust scale if needed
  astronaut.position.set(orbitRadius, -0.5, 0); // Adjust position in the scene
  astronaut.rotation.y = Math.PI / 2;

  astronautPivot.add(astronaut); // Attach astronaut to pivot
  scene.add(astronautPivot); // Add pivot to scene
  // scene.add(astronaut);

  // Update animation loop to rotate the pivot
  function animateOrbit(t = 0) {
    requestAnimationFrame(animateOrbit);
    astronautPivot.rotation.y += orbitSpeed; // Rotate the astronaut around the sun
  }
  animateOrbit();
});
...
function onClick(event) {
  ...
  // Check if the astronaut was clicked
  const textContainer = document.querySelector(".text");

  if (astronaut) {
    const intersects = raycaster.intersectObject(astronaut, true);
    if (intersects.length > 0) {
      // alert("Astronaut clicked! 🚀");
      // fetch("http://api.open-notify.org/astros.json")
      //   .then((response) => response.json())
      //   .then((data) => {
      //     console.log(`Number of astronauts in space: ${data.number}`);
      //     textContainer.innerHTML = `There are currently ${data.number} people in space`;
      //   })
      //   .catch((error) => {
      //     console.error("Error fetching data:", error);
      //   });
      textContainer.innerHTML = `There are currently ${astronautCount} people in space`;
    } else {
...
function initScene(data) {
  ...
  const mercury = getPlanet({ size: 0.1, distance: 2, img: "mercury.png" });
  // const mercury = getPlanet({ size: 0.1, distance: 1.25, img: 'mercury.png' });
  solarSystem.add(mercury);

  const venus = getPlanet({ size: 0.2, distance: 3.5, img: "venus.png" });
  ...
  const mars = getPlanet({ size: 0.15, distance: 6.5, img: "mars.png" });
  ...
  const jupiter = getPlanet({ size: 0.4, distance: 11, img: "jupiter.png" });
  ...
  const neptune = getPlanet({ size: 0.3, distance: 15.5, img: "neptune.png" });
